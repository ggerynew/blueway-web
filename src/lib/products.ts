...
/**
 * Brit → amerikai írásmód. Csak azok a szópárok szerepelnek, amelyeknél a
 * két angol eltér, és a jelentés nem dönt: a „licence/license" és a
 * „practise/practice" főnév-ige megkülönböztetése brit oldalon értelmes, az
 * amerikai mindkettőt egy alakba vonja, tehát az irány egyértelmű.
 *
 * A „metre → meter" és a „programme → program" azért fér bele, mert brit
 * szövegben a másik jelentés más alakban áll (a mérőműszer ott is „meter", a
 * számítógépes program ott is „program"), tehát a csere nem ronthat el
 * jelentést.
 */
const BRIT_AMERIKAI: Record<string, string> = {
  ...
  galvanised: 'galvanized', anodised: 'anodized', pressurised: 'pressurized',
  sterilised: 'sterilized', vulcanised: 'vulcanized', characterised: 'characterized',
  ...
  signalling: 'signaling', travelling: 'traveling', travelled: 'traveled',
...
const BRIT_SZO = new RegExp(`\\b(${Object.keys(BRIT_AMERIKAI).join('|')})\\b`, 'gi');

/**
 * A brit szöveg amerikai írásmódra állítása.
 *
 * Azért itt dől el és nem rekordonként kézzel, mert a két angol változat
 * néhány tucat szóban tér el, minden másban szó szerint egyezik. Ha a
 * különbséget minden mondatnál külön `us` mezőbe másolnánk, a szövegek
 * lassan szétcsúsznának: az egyiket javítanánk, a másikat nem. Így viszont
 * a brit szöveg az egyetlen forrás, az amerikai pedig mindig követi.
 */
export function amerikaiIrasmod(szoveg: string): string {
  return szoveg.replace(BRIT_SZO, (szo) => {
    const uj = BRIT_AMERIKAI[szo.toLowerCase()];
    if (!uj) return szo;
    if (szo === szo.toUpperCase()) return uj.toUpperCase();
    if (szo[0] === szo[0].toUpperCase()) return uj[0].toUpperCase() + uj.slice(1);
    return uj;
  });
}
  ...
  const fill = (node: unknown): unknown => {
    ...
    if (node && typeof node === 'object') {
      ...
      if (typeof rec.hu === 'string' && typeof rec.en === 'string') {
        // Az amerikai változat a britből indul, de nem szó szerint: az
        // írásmódot a szótár igazítja át. Ahol a két piac szóhasználata is
        // más (nem csak az írásmód), ott a rekord saját `us` mezője írja felül.
        return {
          de: rec.en, ko: rec.en, zh: rec.en, it: rec.en, es: rec.en,
          us: amerikaiIrasmod(rec.en),
          ...rec,
        };
      }
...
const categoriesSource: Sourced<Category[]> = [
  ...
  {
    slug: 'cimkezo-gepek',
    name: { hu: 'Címkéző gépek', en: 'Labelling machines', it: 'Etichettatrici', es: 'Etiquetadoras', de: 'Etikettiermaschinen', ko: '라벨링 머신', zh: '贴标机' },
    description: {
      hu: 'Nyomtató-felrakó kombinációk és automata pneumatikus címkéző egységek.',
      en: 'Print-and-apply combinations and automatic pneumatic labelling units.', it: 'Combinazioni stampa e applica e unità di etichettatura pneumatiche automatiche.', es: 'Combinaciones de imprimir y aplicar y unidades de etiquetado neumáticas automáticas.', de: 'Print-and-Apply-Kombinationen und automatische pneumatische Etikettiereinheiten.', ko: '프린트 & 어플라이 복합 시스템 및 자동 공압식 라벨링 유닛.', zh: '打印贴标一体系统与自动气动贴标单元。',
    },
  },
  {
...
const productsSource: Sourced<Product[]> = [
  ...
  },
  {
    ...
    features: [
      ...
      { hu: 'Balra igazított anyagvezetés, alumínium mechanika', en: 'Left-aligned media guide, aluminium mechanism', us: 'Left-aligned media guide, aluminum mechanism', it: 'Guida materiale allineata a sinistra, meccanismo in alluminio', es: 'Guía de material alineada a la izquierda, mecanismo de aluminio', de: 'Linksbündige Materialführung, Aluminiummechanik', ko: '좌측 정렬 미디어 가이드, 알루미늄 메커니즘', zh: '左对齐介质导向，铝制机构' },
  ...
  },
  {
    ...
    features: [
      ...
      { hu: 'Középre igazított anyagvezetés', en: 'Centred media guide', it: 'Guida materiale centrata', es: 'Guía de material centrada', de: 'Zentrierte Materialführung', ko: '중앙 정렬 미디어 가이드', zh: '居中介质导向' },
  ...
  },
  {
    ...
    category: 'cimkenyomtatok',
    name: 'CAB SQUIX Linerless',
    brand: 'CAB',
    image: '/images/products/cab-squix-linerless.webp',
    datasheet: '/datasheets/cab-squix-linerless.pdf',
    short: {
      hu: 'Ipari nyomtató hordozó nélküli címkékhez — 60%-kal több címke egy tekercsen, hulladék nélkül.',
      en: 'Industrial printer for linerless labels — 60% more labels per roll, with no liner waste.', it: 'Stampante industriale per etichette linerless — il 60% di etichette in più per rotolo, senza scarti di liner.', es: 'Impresora industrial para etiquetas linerless: un 60 % más de etiquetas por rollo y sin residuos de liner.', de: 'Industriedrucker für linerlose Etiketten — 60 % mehr Etiketten pro Rolle, ohne Trägerabfall.', ko: '라이너리스 라벨용 산업용 프린터 — 롤당 라벨 수 60% 증가, 이형지 폐기물 없음.', zh: '面向无底纸标签的工业打印机——每卷标签数量增加 60%，且没有底纸废料。',
    },
    description: {
      hu: 'A hordozó nélküli címkénél a ragasztó közvetlenül a szalagon van, szilikonozott hátlap nincs. A cab adatai szerint így 60 százalékkal több címke fér egy tekercsre, mint hordozós anyagból — és nincs mit elszállítani, mert nincs hordozó-hulladék.\n\nA címkéket a beépített vágó választja el, méghozzá eltérő hosszakra. Az anyagtakarékos üzem programozható: a gép az utolsó nyomtatott sor után egy megadott ráhagyással vág, tehát minden címke csak annyi anyagot használ, amennyit a tartalma megkíván — kivágott, kötött méretű címkénél ez nem megoldható.\n\nA gép működésében és méreteiben teljesen illeszkedik a szabvány SQUIX nyomtatókhoz, és a meglévő alap- és adagolós gépek utólag is átalakíthatók: az átalakító készlet egy fedőlemezből, a nyomóhenger melletti vezetőlemezből, a belső visszatekercselő fölé kerülő védőcsőből, egy vágóból és egy linerless nyomóhengerből áll.\n\nMűszaki adatok: direkt termikus nyomtatás — a hőnyomtatáshoz kisebb fejnyomás kell, ez a nyomtatófej élettartamának is jót tesz. Felbontás 203 vagy 300 dpi, sebesség 250 mm/s, anyag direkt termikus papír, legalább 30 mm címkemagasság, legfeljebb 110 µm vastagság, tekercs 205 mm külső átmérőig, 38,1–76 mm magátmérővel. A vágó gyárilag jár hozzá: edzett, titánbevonatos kés, ragasztónak ellenálló felülettel, és külön érzékelő figyeli a levágott címke leszedését.',
      en: 'On a linerless label the adhesive sits on the web itself; there is no siliconised backing. According to cab, 60 percent more labels fit on a roll than with lined material — and there is nothing to haul away, because there is no liner waste.\n\nA built-in cutter separates the labels, and it can cut them to different lengths. The material-saving function is programmable: the printer cuts after the final print line plus a set offset, so every label uses only as much material as its content needs — something die-cut, fixed-size labels cannot do.\n\nIn function and dimensions the machine matches the standard SQUIX printers entirely, and existing basic and peel-off units can be retrofitted: the kit consists of a cover plate, a guide plate next to the print roller, a protective tube above the internal rewinder, a cutter and a linerless print roller.\n\nTechnical data: direct thermal printing — thermal printing needs less print head pressure, which also lengthens the life of the head. Resolution 203 or 300 dpi, speed 250 mm/s, material direct thermal paper, label height at least 30 mm, thickness up to 110 µm, roll up to 205 mm outside diameter on a 38.1–76 mm core. The cutter is included: a hardened, titanium-coated blade with an adhesive-resistant surface, and a separate sensor watches for the cut label being taken.', us: 'On a linerless label the adhesive sits on the web itself; there is no siliconized backing. According to cab, 60 percent more labels fit on a roll than with lined material — and there is nothing to haul away, because there is no liner waste.\n\nA built-in cutter separates the labels, and it can cut them to different lengths. The material-saving function is programmable: the printer cuts after the final print line plus a set offset, so every label uses only as much material as its content needs — something die-cut, fixed-size labels cannot do.\n\nIn function and dimensions the machine matches the standard SQUIX printers entirely, and existing basic and peel-off units can be retrofitted: the kit consists of a cover plate, a guide plate next to the print roller, a protective tube above the internal rewinder, a cutter and a linerless print roller.\n\nTechnical data: direct thermal printing — thermal printing needs less print head pressure, which also lengthens the life of the head. Resolution 203 or 300 dpi, speed 250 mm/s, material direct thermal paper, label height at least 30 mm, thickness up to 110 µm, roll up to 205 mm outside diameter on a 38.1–76 mm core. The cutter is included: a hardened, titanium-coated blade with an adhesive-resistant surface, and a separate sensor watches for the cut label being taken.', it: 'In un’etichetta linerless l’adesivo si trova sul nastro stesso, senza supporto siliconato. Secondo cab, su un rotolo entrano il 60 percento di etichette in più rispetto al materiale con liner — e non c’è nulla da smaltire, perché non c’è scarto di liner.\n\nUna taglierina integrata separa le etichette e può tagliarle a lunghezze diverse. La funzione di risparmio materiale è programmabile: la stampante taglia dopo l’ultima riga stampata più un offset impostato, così ogni etichetta usa solo il materiale richiesto dal suo contenuto — cosa impossibile con etichette fustellate di formato fisso.\n\nPer funzionamento e dimensioni la macchina corrisponde interamente alle stampanti SQUIX standard, e le unità base e con spellicolatore già esistenti possono essere riadattate: il kit comprende una piastra di copertura, una piastra di guida accanto al rullo di stampa, un tubo protettivo sopra il riavvolgitore interno, una taglierina e un rullo di stampa linerless.\n\nDati tecnici: stampa termica diretta — la stampa termica richiede meno pressione sulla testina, il che ne allunga anche la durata. Risoluzione 203 o 300 dpi, velocità 250 mm/s, materiale carta termica diretta, altezza etichetta almeno 30 mm, spessore fino a 110 µm, rotolo fino a 205 mm di diametro esterno su anima da 38,1–76 mm. La taglierina è compresa: lama temprata e rivestita in titanio, con superficie resistente all’adesivo, e un sensore dedicato controlla il prelievo dell’etichetta tagliata.', es: 'En una etiqueta linerless el adhesivo está en la propia banda; no hay soporte siliconado. Según cab, en un rollo caben un 60 por ciento más de etiquetas que con material sobre liner, y no hay nada que retirar, porque no hay residuo de liner.\n\nUna cizalla integrada separa las etiquetas, y puede cortarlas a longitudes distintas. La función de ahorro de material es programable: la impresora corta tras la última línea impresa más un desplazamiento definido, de modo que cada etiqueta usa solo el material que su contenido necesita, algo que las etiquetas troqueladas de formato fijo no permiten.\n\nEn funcionamiento y dimensiones la máquina coincide por completo con las impresoras SQUIX estándar, y las unidades básicas y de despegado existentes pueden readaptarse: el kit consta de una placa de cubierta, una placa guía junto al rodillo de impresión, un tubo protector sobre el rebobinador interno, una cizalla y un rodillo de impresión linerless.\n\nDatos técnicos: impresión térmica directa; la impresión térmica requiere menos presión del cabezal, lo que además alarga su vida útil. Resolución de 203 o 300 dpi, velocidad de 250 mm/s, material papel térmico directo, altura de etiqueta de al menos 30 mm, espesor hasta 110 µm, rollo de hasta 205 mm de diámetro exterior sobre núcleo de 38,1–76 mm. La cizalla viene incluida: cuchilla templada y recubierta de titanio, con superficie resistente al adhesivo, y un sensor específico vigila la retirada de la etiqueta cortada.', de: 'Bei einem linerlosen Etikett sitzt der Klebstoff auf der Bahn selbst; ein silikonisiertes Trägerpapier gibt es nicht. Laut cab passen so 60 Prozent mehr Etiketten auf eine Rolle als bei Trägermaterial — und es bleibt nichts zu entsorgen, weil kein Trägerabfall anfällt.\n\nEin integriertes Messer trennt die Etiketten, und zwar auch auf unterschiedliche Längen. Die Materialsparfunktion ist programmierbar: Der Drucker schneidet nach der letzten Druckzeile plus einem eingestellten Offset, jedes Etikett verbraucht also nur so viel Material, wie sein Inhalt verlangt — bei gestanzten Etiketten fester Größe ist das nicht möglich.\n\nIn Funktion und Abmessungen entspricht die Maschine vollständig den Standard-SQUIX-Druckern, und vorhandene Basis- und Spendegeräte lassen sich nachrüsten: Der Bausatz besteht aus einem Abdeckblech, einem Führungsblech neben der Druckwalze, einem Schutzrohr über dem internen Aufwickler, einem Messer und einer linerlosen Druckwalze.\n\nTechnische Daten: Thermodirektdruck — Thermodruck braucht weniger Druckkopfanpressung, was auch die Lebensdauer des Kopfes verlängert. Auflösung 203 oder 300 dpi, Geschwindigkeit 250 mm/s, Material Thermodirektpapier, Etikettenhöhe mindestens 30 mm, Dicke bis 110 µm, Rolle bis 205 mm Außendurchmesser auf 38,1–76-mm-Kern. Das Messer gehört zum Lieferumfang: gehärtete, titanbeschichtete Klinge mit klebstoffresistenter Oberfläche, und ein eigener Sensor überwacht die Entnahme des geschnittenen Etiketts.', ko: '라이너리스 라벨은 접착제가 웹 자체에 있으며, 실리콘 처리된 이형지가 없습니다. cab의 자료에 따르면 이형지가 있는 소재보다 롤당 라벨을 60퍼센트 더 감을 수 있으며, 이형지 폐기물이 없으므로 처리할 것도 없습니다.\n\n내장 커터가 라벨을 분리하며, 서로 다른 길이로도 절단할 수 있습니다. 소재 절약 기능은 프로그래밍이 가능합니다. 프린터는 마지막 인쇄 라인 이후 설정된 오프셋만큼 지나 절단하므로, 모든 라벨은 내용이 요구하는 만큼의 소재만 사용합니다. 크기가 고정된 타발 라벨로는 불가능한 일입니다.\n\n기능과 치수 면에서 표준 SQUIX 프린터와 완전히 호환되며, 기존의 기본형과 필오프 장비도 개조할 수 있습니다. 개조 키트는 커버 플레이트, 인쇄 롤러 옆 가이드 플레이트, 내장 리와인더 위의 보호 튜브, 커터, 라이너리스 인쇄 롤러로 구성됩니다.\n\n기술 사양: 감열 직접 인쇄 — 감열 방식은 헤드 압력이 덜 필요하여 프린트 헤드 수명에도 유리합니다. 해상도 203 또는 300 dpi, 속도 250 mm/s, 소재는 감열지, 라벨 높이 최소 30 mm, 두께 최대 110 µm, 롤 외경 최대 205 mm, 코어 38.1~76 mm. 커터는 기본 제공됩니다. 경화 처리된 티타늄 코팅 칼날에 접착제에 강한 표면을 갖추었고, 별도의 센서가 절단된 라벨의 제거 여부를 감시합니다.', zh: '无底纸标签的胶粘剂直接位于料带上，没有硅油底纸。按 cab 的数据，同样一卷可比带底纸的材料多容纳 60% 的标签——而且没有底纸废料，也就无需清运。\n\n内置切刀负责分离标签，并且可以切成不同长度。省料功能可编程：打印机在最后一行打印内容之后按设定的偏移量切断，因此每张标签只消耗其内容所需的材料——这是尺寸固定的模切标签做不到的。\n\n在功能与尺寸上，本机与标准 SQUIX 打印机完全一致，现有的基本型与剥离型设备也可以后期加装：改装套件包含一块盖板、打印胶辊旁的导向板、内部回卷器上方的保护管、一把切刀和一支无底纸打印胶辊。\n\n技术参数：热敏直接打印——热敏方式所需打印头压力更小，也有利于打印头寿命。分辨率 203 或 300 dpi，速度 250 mm/s，材料为热敏纸，标签高度至少 30 mm，厚度最大 110 µm，卷料外径最大 205 mm，纸芯 38.1–76 mm。切刀为标准配置：淬硬并带钛涂层的刀片，表面抗胶粘，另有独立传感器监测已切标签是否被取走。',
    },
    features: [
      { hu: '60%-kal több címke egy tekercsen, hordozó-hulladék nélkül', en: '60% more labels per roll, with no liner waste', it: 'Il 60% di etichette in più per rotolo, senza scarti di liner', es: 'Un 60 % más de etiquetas por rollo y sin residuos de liner', de: '60 % mehr Etiketten pro Rolle, ohne Trägerabfall', ko: '롤당 라벨 수 60% 증가, 이형지 폐기물 없음', zh: '每卷标签数量增加 60%，且没有底纸废料' },
      { hu: 'Beépített vágó — a címkék eltérő hosszúságúak lehetnek', en: 'Built-in cutter — labels can differ in length', it: 'Taglierina integrata — le etichette possono avere lunghezze diverse', es: 'Cizalla integrada: las etiquetas pueden tener longitudes distintas', de: 'Integriertes Messer — Etiketten dürfen unterschiedlich lang sein', ko: '내장 커터 — 라벨 길이를 다르게 할 수 있음', zh: '内置切刀——标签长度可以各不相同' },
      { hu: 'Programozható anyagtakarékos üzem', en: 'Programmable material-saving function', it: 'Funzione di risparmio materiale programmabile', es: 'Función de ahorro de material programable', de: 'Programmierbare Materialsparfunktion', ko: '프로그래밍 가능한 소재 절약 기능', zh: '可编程的省料功能' },
      { hu: 'Direkt termikus, 203 / 300 dpi, 250 mm/s', en: 'Direct thermal, 203 / 300 dpi, 250 mm/s', it: 'Termica diretta, 203 / 300 dpi, 250 mm/s', es: 'Térmica directa, 203 / 300 dpi, 250 mm/s', de: 'Thermodirekt, 203 / 300 dpi, 250 mm/s', ko: '감열 직접 인쇄, 203 / 300 dpi, 250 mm/s', zh: '热敏直接打印，203 / 300 dpi，250 mm/s' },
      { hu: 'Meglévő alap és adagolós SQUIX utólag átalakítható', en: 'Existing basic and peel-off SQUIX units can be retrofitted', it: 'Le unità SQUIX base e con spellicolatore già esistenti sono riadattabili', es: 'Las unidades SQUIX básicas y de despegado existentes pueden readaptarse', de: 'Vorhandene Basis- und Spende-SQUIX lassen sich nachrüsten', ko: '기존 기본형 및 필오프 SQUIX 개조 가능', zh: '现有的基本型与剥离型 SQUIX 可后期加装' },
      { hu: 'Edzett, titánbevonatos kés; a leszedést érzékelő figyeli', en: 'Hardened, titanium-coated blade; a sensor watches label removal', it: 'Lama temprata e rivestita in titanio; un sensore controlla il prelievo', es: 'Cuchilla templada y recubierta de titanio; un sensor vigila la retirada', de: 'Gehärtete, titanbeschichtete Klinge; ein Sensor überwacht die Entnahme', ko: '경화 티타늄 코팅 칼날, 라벨 제거를 센서가 감시', zh: '淬硬钛涂层刀片；由传感器监测标签取走' },
    ],
    orientation: {
      image: '/images/applicators/squix-linerless-vago.webp',
      title: {
        hu: 'A vágó a hordozó helyett',
        en: 'The cutter in place of the liner', it: 'La taglierina al posto del liner', es: 'La cizalla en lugar del liner', de: 'Das Messer statt des Trägers', ko: '이형지를 대신하는 커터', zh: '用切刀取代底纸',
      },
      text: {
        hu: 'Hordozós anyagnál a címke alakját a kivágás adja, tehát minden címke egyforma. Hordozó nélkül nincs kivágás: a hosszt a vágó szabja meg, méghozzá nyomtatási feladatonként külön. A képen a SQUIX Linerless 6.3 a CU600L vágóval; a 4.3-as gép a CSQL vágót kapja. A kés edzett és titánbevonatos, a felülete ellenáll a ragasztónak — ez a hordozó nélküli nyomtatás legjobban igénybe vett alkatrésze.',
        en: 'With lined material the shape of the label comes from the die cut, so every label is the same. Without a liner there is no die cut: the length is set by the cutter, and it can differ from print job to print job. The picture shows the SQUIX Linerless 6.3 with the CU600L cutter; the 4.3 gets the CSQL cutter. The blade is hardened and titanium-coated and its surface resists adhesive — this is the most heavily worked part in linerless printing.', it: 'Con il materiale su liner la forma dell’etichetta deriva dalla fustellatura, quindi tutte le etichette sono uguali. Senza liner non c’è fustellatura: la lunghezza la definisce la taglierina, e può cambiare da un lavoro di stampa all’altro. Nell’immagine la SQUIX Linerless 6.3 con la taglierina CU600L; la 4.3 monta la taglierina CSQL. La lama è temprata e rivestita in titanio e la sua superficie resiste all’adesivo: è il componente più sollecitato nella stampa senza liner.', es: 'Con material sobre liner la forma de la etiqueta la da el troquelado, así que todas las etiquetas son iguales. Sin liner no hay troquelado: la longitud la fija la cizalla, y puede variar de un trabajo de impresión a otro. En la imagen, la SQUIX Linerless 6.3 con la cizalla CU600L; la 4.3 lleva la cizalla CSQL. La cuchilla está templada y recubierta de titanio y su superficie resiste al adhesivo: es la pieza más exigida en la impresión sin liner.', de: 'Bei Trägermaterial gibt die Stanzung die Form des Etiketts vor, alle Etiketten sind also gleich. Ohne Träger gibt es keine Stanzung: Die Länge bestimmt das Messer, und zwar je Druckauftrag unterschiedlich. Im Bild der SQUIX Linerless 6.3 mit dem Messer CU600L; der 4.3 erhält das Messer CSQL. Die Klinge ist gehärtet und titanbeschichtet, ihre Oberfläche widersteht dem Klebstoff — das am stärksten beanspruchte Teil im linerlosen Druck.', ko: '이형지가 있는 소재에서는 라벨의 모양을 타발이 결정하므로 모든 라벨이 동일합니다. 이형지가 없으면 타발도 없습니다. 길이는 커터가 정하며, 인쇄 작업마다 다르게 할 수 있습니다. 사진은 CU600L 커터를 장착한 SQUIX Linerless 6.3이며, 4.3 기종에는 CSQL 커터가 적용됩니다. 칼날은 경화 처리 후 티타늄 코팅되어 있고 표면이 접착제에 강합니다. 라이너리스 인쇄에서 가장 부하가 큰 부품입니다.', zh: '使用带底纸的材料时，标签形状由模切决定，因此每张标签都一样。没有底纸就没有模切：长度由切刀决定，而且可以逐个打印任务不同。图中为配 CU600L 切刀的 SQUIX Linerless 6.3；4.3 机型配 CSQL 切刀。刀片经淬硬并带钛涂层，表面抗胶粘——这是无底纸打印中负荷最大的部件。',
      },
    },
    variantsLead: {
      hu: 'Két gépszélesség, minden másban azonosak — a címke szélessége dönt.',
      en: 'Two machine widths, identical in everything else — the label width decides.', it: 'Due larghezze di macchina, identiche in tutto il resto: decide la larghezza dell’etichetta.', es: 'Dos anchos de máquina, idénticos en todo lo demás: decide el ancho de la etiqueta.', de: 'Zwei Maschinenbreiten, in allem Übrigen identisch — die Etikettenbreite entscheidet.', ko: '두 가지 기계 폭이며 나머지는 동일합니다. 라벨 폭이 선택을 좌우합니다.', zh: '两种机器宽度，其余完全相同——由标签宽度决定。',
    },
    variants: [
      {
        name: 'SQUIX 4.3 L',
        purpose: {
          hu: 'A 4"-es gép: 20–120 mm széles anyaghoz, CSQL vágóval. A hordozó nélküli címkék többsége ebbe a méretbe esik.',
          en: 'The 4" machine: for 20–120 mm wide material, with the CSQL cutter. Most linerless labels fall into this size.', it: 'La macchina da 4": per materiale largo 20–120 mm, con taglierina CSQL. La maggior parte delle etichette linerless rientra in questa misura.', es: 'La máquina de 4": para material de 20–120 mm de ancho, con cizalla CSQL. La mayoría de las etiquetas linerless caben en este tamaño.', de: 'Die 4"-Maschine: für 20–120 mm breites Material, mit dem Messer CSQL. Die meisten linerlosen Etiketten fallen in diese Größe.', ko: '4" 기종: 폭 20~120 mm 소재용, CSQL 커터 장착. 대부분의 라이너리스 라벨이 이 크기에 해당합니다.', zh: '4 英寸机型：适用 20–120 mm 宽的材料，配 CSQL 切刀。多数无底纸标签都在这个尺寸范围内。',
        },
        params: [
          { label: { hu: 'Felbontás / nyomtatási szélesség', en: 'Resolution / print width', it: 'Risoluzione / larghezza di stampa', es: 'Resolución / ancho de impresión', de: 'Auflösung / Druckbreite', ko: '해상도 / 인쇄 폭', zh: '分辨率 / 打印宽度' }, value: { hu: '203 dpi → 104 mm, 300 dpi → 108,4 mm', en: '203 dpi → 104 mm, 300 dpi → 108.4 mm', it: '203 dpi → 104 mm, 300 dpi → 108,4 mm', es: '203 dpi → 104 mm, 300 dpi → 108,4 mm', de: '203 dpi → 104 mm, 300 dpi → 108,4 mm', ko: '203 dpi → 104 mm, 300 dpi → 108.4 mm', zh: '203 dpi → 104 mm，300 dpi → 108.4 mm' } },
          { label: { hu: 'Anyagszélesség', en: 'Material width', it: 'Larghezza materiale', es: 'Ancho de material', de: 'Materialbreite', ko: '소재 폭', zh: '材料宽度' }, value: { hu: '20–120 mm', en: '20–120 mm', it: '20–120 mm', es: '20–120 mm', de: '20–120 mm', ko: '20~120 mm', zh: '20–120 mm' } },
          { label: { hu: 'Vágó', en: 'Cutter', it: 'Taglierina', es: 'Cizalla', de: 'Messer', ko: '커터', zh: '切刀' }, value: { hu: 'CSQL', en: 'CSQL', it: 'CSQL', es: 'CSQL', de: 'CSQL', ko: 'CSQL', zh: 'CSQL' } },
          { label: { hu: 'Méret / tömeg', en: 'Dimensions / weight', it: 'Dimensioni / peso', es: 'Dimensiones / peso', de: 'Maße / Gewicht', ko: '치수 / 무게', zh: '尺寸 / 重量' }, value: { hu: '252 × 288 × 460 mm, kb. 10 kg', en: '252 × 288 × 460 mm, approx. 10 kg', it: '252 × 288 × 460 mm, ca. 10 kg', es: '252 × 288 × 460 mm, aprox. 10 kg', de: '252 × 288 × 460 mm, ca. 10 kg', ko: '252 × 288 × 460 mm, 약 10 kg', zh: '252 × 288 × 460 mm，约 10 kg' } },
        ],
      },
      {
        name: 'SQUIX 6.3 L',
        purpose: {
          hu: 'A 6"-es gép: 46–180 mm széles anyaghoz, CU600L vágóval. Széles szállítási és raklapcímkékhez.',
          en: 'The 6" machine: for 46–180 mm wide material, with the CU600L cutter. For wide shipping and pallet labels.', it: 'La macchina da 6": per materiale largo 46–180 mm, con taglierina CU600L. Per etichette di spedizione e da pallet larghe.', es: 'La máquina de 6": para material de 46–180 mm de ancho, con cizalla CU600L. Para etiquetas de envío y de palé anchas.', de: 'Die 6"-Maschine: für 46–180 mm breites Material, mit dem Messer CU600L. Für breite Versand- und Palettenetiketten.', ko: '6" 기종: 폭 46~180 mm 소재용, CU600L 커터 장착. 폭이 넓은 배송 라벨과 팔레트 라벨용입니다.', zh: '6 英寸机型：适用 46–180 mm 宽的材料，配 CU600L 切刀。用于宽幅运单标签与托盘标签。',
        },
        params: [
          { label: { hu: 'Felbontás / nyomtatási szélesség', en: 'Resolution / print width', it: 'Risoluzione / larghezza di stampa', es: 'Resolución / ancho de impresión', de: 'Auflösung / Druckbreite', ko: '해상도 / 인쇄 폭', zh: '分辨率 / 打印宽度' }, value: { hu: '203 dpi → 168 mm, 300 dpi → 162,6 mm', en: '203 dpi → 168 mm, 300 dpi → 162.6 mm', it: '203 dpi → 168 mm, 300 dpi → 162,6 mm', es: '203 dpi → 168 mm, 300 dpi → 162,6 mm', de: '203 dpi → 168 mm, 300 dpi → 162,6 mm', ko: '203 dpi → 168 mm, 300 dpi → 162.6 mm', zh: '203 dpi → 168 mm，300 dpi → 162.6 mm' } },
          { label: { hu: 'Anyagszélesség', en: 'Material width', it: 'Larghezza materiale', es: 'Ancho de material', de: 'Materialbreite', ko: '소재 폭', zh: '材料宽度' }, value: { hu: '46–180 mm', en: '46–180 mm', it: '46–180 mm', es: '46–180 mm', de: '46–180 mm', ko: '46~180 mm', zh: '46–180 mm' } },
          { label: { hu: 'Vágó', en: 'Cutter', it: 'Taglierina', es: 'Cizalla', de: 'Messer', ko: '커터', zh: '切刀' }, value: { hu: 'CU600L', en: 'CU600L', it: 'CU600L', es: 'CU600L', de: 'CU600L', ko: 'CU600L', zh: 'CU600L' } },
          { label: { hu: 'Méret / tömeg', en: 'Dimensions / weight', it: 'Dimensioni / peso', es: 'Dimensiones / peso', de: 'Maße / Gewicht', ko: '치수 / 무게', zh: '尺寸 / 重量' }, value: { hu: '312 × 288 × 460 mm, kb. 14 kg', en: '312 × 288 × 460 mm, approx. 14 kg', it: '312 × 288 × 460 mm, ca. 14 kg', es: '312 × 288 × 460 mm, aprox. 14 kg', de: '312 × 288 × 460 mm, ca. 14 kg', ko: '312 × 288 × 460 mm, 약 14 kg', zh: '312 × 288 × 460 mm，约 14 kg' } },
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    features: [
      ...
      { hu: 'Színes LCD-kijelző és navigátor-panel', en: 'Coloured LCD display and navigator pad', it: 'Display LCD a colori e navigator pad', es: 'Display LCD en color y pad de navegación', de: 'Farbiges LCD-Display und Navigator-Pad', ko: '컬러 LCD 디스플레이 및 내비게이터 패드', zh: '彩色 LCD 显示屏与导航键盘' },
      { hu: 'Nyomtatási szélesség 108 mm (4")', en: 'Print width 108 mm (4")', it: 'Larghezza di stampa 108 mm (4")', es: 'Ancho de impresión 108 mm (4")', de: 'Druckbreite 108 mm (4")', ko: '인쇄 폭 108 mm (4")', zh: '打印宽度 108 mm（4"）' },
      { hu: 'Termotranszfer és direkt termál', en: 'Thermal transfer and direct thermal', it: 'Trasferimento termico e termico diretto', es: 'Transferencia térmica y térmico directo', de: 'Thermotransfer- und Thermodirektdruck', ko: '열전사 및 감열 방식', zh: '热转印与热敏打印' },
      { hu: 'RS232, USB, Ethernet; cablabel S3', en: 'RS232, USB, Ethernet; cablabel S3', it: 'RS232, USB, Ethernet; cablabel S3', es: 'RS232, USB, Ethernet; cablabel S3', de: 'RS232, USB, Ethernet; cablabel S3', ko: 'RS232, USB, Ethernet; cablabel S3', zh: 'RS232、USB、Ethernet；cablabel S3' },
    ],
  ...
  },
  {
    ...
    features: [
      ...
      { hu: 'Középre igazított anyagvezetés, elölről fűzés', en: 'Centred media guide, front loading', it: 'Guida materiale centrata, caricamento frontale', es: 'Guía de material centrada, carga frontal', de: 'Zentrierte Materialführung, Einlegen von vorn', ko: '중앙 정렬 미디어 가이드, 전면 장착', zh: '居中介质导向，正面装载' },
      { hu: 'Nagy színes érintőkijelző', en: 'Large coloured touch display', it: 'Ampio display touch a colori', es: 'Gran pantalla táctil en color', de: 'Großes farbiges Touchdisplay', ko: '대형 컬러 터치 디스플레이', zh: '大尺寸彩色触摸屏' },
  ...
  },
  {
    ...
    features: [
      ...
      { hu: 'GHS-megfelelő címkézés', en: 'GHS-compliant labelling', it: 'Etichettatura conforme GHS', es: 'Etiquetado conforme a GHS', de: 'GHS-konforme Kennzeichnung', ko: 'GHS 규정 준수 라벨링', zh: '符合 GHS 的标签打印' },
    ],
    ...
    extraVideos: [
      {
        videoId: 'HTGYb6krA6w',
        label: { hu: 'Kétszínű nyomtatás egy menetben', en: 'Two-colour printing in one pass', de: 'Zweifarbiger Druck in einem Durchlauf', it: 'Stampa a due colori in un unico passaggio', es: 'Impresión a dos colores en una sola pasada', ko: '한 번의 공정으로 2색 인쇄', zh: '一次走纸完成双色打印' },
      },
    ],
  },
  {
    ...
    short: {
      ...
      en: 'Two-colour printing with a 6" head — for GHS hazard labels.', us: 'Two-color printing with a 6" head — for GHS hazard labels.', it: 'Stampa bicolore con testina da 6" — per etichette di pericolo GHS.', es: 'Impresión a dos colores con cabezal de 6" — para etiquetas de peligro GHS.', de: 'Zweifarbiger Druck mit 6"-Kopf — für GHS-Gefahrstoffetiketten.', ko: '6" 헤드를 갖춘 2색 인쇄 — GHS 위험물 라벨용.', zh: '配备 6" 打印头的双色打印——适用于 GHS 危险品标签。',
    },
    description: {
    ...
    applicators: [
      ...
      },
      {
        ...
        description: {
          ...
          en: 'For simple labelling. Two versions: for 4" and 6" width.', it: 'Per un’etichettatura semplice. Due versioni: per larghezza 4" e 6".', es: 'Para un etiquetado sencillo. Dos versiones: para anchura de 4" y 6".', de: 'Für einfache Etikettierung. Zwei Ausführungen: für 4" und 6" Breite.', ko: '간단한 라벨링용. 4"와 6" 폭용 두 가지 버전.', zh: '适用于简单贴标任务。 两种版本：适用于 4" 和 6" 宽度。',
        },
        longDescription: {
      ...
      },
      {
        ...
        description: {
          ...
          en: 'High-precision labelling of cables.', it: 'Etichettatura ad alta precisione di cavi.', es: 'Etiquetado de alta precisión de cables.', de: 'Hochpräzise Etikettierung von Kabeln.', ko: '케이블의 고정밀 라벨링.', zh: '线缆的高精度贴标。',
        },
        longDescription: {
      ...
      },
      {
        ...
        description: {
          ...
          en: 'High-speed labelling of products in motion. Two versions: for 4" and 6" width.', it: 'Etichettatura ad alta velocità di prodotti in movimento. Due versioni: per larghezza 4" e 6".', es: 'Etiquetado a alta velocidad de productos en movimiento. Dos versiones: para anchura de 4" y 6".', de: 'Hochgeschwindigkeits-Etikettierung bewegter Produkte. Zwei Ausführungen: für 4" und 6" Breite.', ko: '이동 중인 제품의 고속 라벨링. 4"와 6" 폭용 두 가지 버전.', zh: '对运动中的产品进行高速贴标。两种版本：适用于 4" 和 6" 宽度。',
        },
        longDescription: {
      ...
      },
      {
        ...
        description: {
          hu: 'Belső címkézés.',
          en: 'Inside labelling.', it: 'Etichettatura interna.', es: 'Etiquetado interior.', de: 'Innenetikettierung.', ko: '내부 라벨링.', zh: '内表面贴标。',
        },
        longDescription: {
  ...
  },
  {
    ...
    name: 'CAB HERMES QL',
    brand: 'CAB',
    image: '/images/products/cab-hermes-ql.webp',
    datasheet: '/datasheets/cab-hermes-ql.pdf',
    short: {
      hu: 'Print & apply hordozó nélküli (linerless) és InNo-Liner anyagokhoz — szilikonos hordozó és hulladék nélkül.',
      en: 'Print & apply for linerless and InNo-Liner materials — no silicone liner, no liner waste.', it: 'Stampa e applica per materiali linerless e InNo-Liner — senza liner siliconato, senza scarti.', es: 'Imprimir y aplicar para materiales linerless e InNo-Liner: sin liner siliconado y sin residuos.', de: 'Print & Apply für linerlose und InNo-Liner-Materialien — ohne silikonisiertes Trägermaterial, ohne Abfall.', ko: '라이너리스 및 InNo-Liner 소재용 프린트 앤 어플라이 — 실리콘 라이너도, 라이너 폐기물도 없습니다.', zh: '面向无底纸（linerless）与 InNo-Liner 材料的即打即贴——没有硅油底纸，也没有底纸废料。',
    },
    description: {
      hu: 'A hordozó nélküli címkénél nincs szilikonozott hátlap: a ragasztó közvetlenül a szalagon van, a gép pedig méretre vágja a címkét. Ezzel egyszerre két dolog szűnik meg — a hordozó hulladéka és az elszállításának költsége, illetve a kötött címkehossz. A HERMES QL az utolsó nyomtatott sor után egy megadott ráhagyással automatikusan vág, tehát minden címke pontosan olyan hosszú, amilyen a tartalma.\n\nEgy alapgép kétféle anyagot kezel. A linerless öntapadó, a felülete szilikonozott, hogy a feltekercselt szalag ne ragadjon önmagához; ehhez tapadásgátló bevonatú DRL nyomóhenger való. Az InNo-Liner ragasztója vízzel aktiválódik, szilikon egyáltalán nincs benne, és nedvszívó karton- vagy papírfelületre való — jellemzően logisztikai szállítási címkékhez; ehhez szilikonbevonatú DRS henger jár.\n\nA gép a bevált HERMES Q-ra épül: a méretek, a belső szoftver, a kezelőfelület, a csatlakozók és az applikátor-illesztés azonosak, az applikátorok és a rögzítő elemek átvihetők a kettő között. A ciklusidő is a HERMES Q-é, kiegészítve nagyjából 50 ezredmásodperccel a vágásra.\n\nTípusok: HERMES QL4.3 és QL6.3, bal és jobb kivitelben, 200 vagy 300 dpi felbontással. Műszaki adatok: direkt termikus nyomtatás, 200 vagy 300 dpi, akár 250 mm/s, nyomtatási szélesség 108,4 mm-ig (168 mm külön kérésre). Anyagszélesség 50–118 vagy 50–178 mm, címkehossz 30–456 mm, vastagság legfeljebb 84 µm. A tekercs külső átmérője 300 mm-ig, magátmérő 76 mm — így akár 800 m anyag fér fel, míg ugyanebbe a gépvázba hordozós anyagból 205 mm-es tekercs és 180 m megy. A vágó szerszám nélkül kivehető tisztításhoz, kése és ellenkése tapadásgátló bevonatú.',
      en: 'A linerless label has no siliconised backing paper: the adhesive is on the web itself and the machine cuts each label to length. Two things disappear at once — the liner waste and the cost of disposing of it, and the fixed label length. The HERMES QL cuts automatically after the final print line plus a set offset, so every label is exactly as long as its content.\n\nOne base unit handles two kinds of material. Linerless is self-adhesive and its surface is siliconised so the wound web does not stick to itself; it takes the DRL print roller with an anti-stick coating. InNo-Liner adhesive is activated by water, contains no silicone at all and is meant for absorbent cardboard or paper surfaces — typically shipping labels in logistics; it takes the silicone-coated DRS roller.\n\nThe machine is built on the proven HERMES Q: dimensions, firmware, user interface, interfaces and applicator connectivity are identical, and applicators and assembly assistances can be moved between the two. Cycle rates are those of the HERMES Q, plus roughly 50 milliseconds for the cut.\n\nModels: HERMES QL4.3 and QL6.3, in left- and right-hand versions, with 200 or 300 dpi. Technical data: direct thermal printing, 200 or 300 dpi, up to 250 mm/s, print width up to 108.4 mm (168 mm on request). Material width 50–118 or 50–178 mm, label height 30–456 mm, thickness up to 84 µm. Roll outside diameter up to 300 mm on a 76 mm core — so up to 800 m of material fits, whereas lined material in the same chassis means a 205 mm roll and 180 m. The cutter can be removed for cleaning without tools; blade and cutter bar are anti-stick coated.', us: 'A linerless label has no siliconized backing paper: the adhesive is on the web itself and the machine cuts each label to length. Two things disappear at once — the liner waste and the cost of disposing of it, and the fixed label length. The HERMES QL cuts automatically after the final print line plus a set offset, so every label is exactly as long as its content.\n\nOne base unit handles two kinds of material. Linerless is pressure-sensitive and its surface is siliconized so the wound web does not stick to itself; it takes the DRL print roller with an anti-stick coating. InNo-Liner adhesive is activated by water, contains no silicone at all and is meant for absorbent cardboard or paper surfaces — typically shipping labels in logistics; it takes the silicone-coated DRS roller.\n\nThe machine is built on the proven HERMES Q: dimensions, firmware, user interface, interfaces and applicator connectivity are identical, and applicators and assembly assistances can be moved between the two. Cycle rates are those of the HERMES Q, plus roughly 50 milliseconds for the cut.\n\nModels: HERMES QL4.3 and QL6.3, in left- and right-hand versions, with 200 or 300 dpi. Technical data: direct thermal printing, 200 or 300 dpi, up to 250 mm/s, print width up to 108.4 mm (168 mm on request). Material width 50–118 or 50–178 mm, label height 30–456 mm, thickness up to 84 µm. Roll outside diameter up to 300 mm on a 76 mm core — so up to 800 m of material fits, whereas lined material in the same chassis means a 205 mm roll and 180 m. The cutter can be removed for cleaning without tools; blade and cutter bar are anti-stick coated.', it: 'Un’etichetta linerless non ha supporto siliconato: l’adesivo è sul nastro stesso e la macchina taglia ogni etichetta a misura. Spariscono due cose insieme — lo scarto del liner e il costo del suo smaltimento, e la lunghezza fissa dell’etichetta. La HERMES QL taglia automaticamente dopo l’ultima riga stampata più un offset impostato, così ogni etichetta è lunga esattamente quanto il suo contenuto.\n\nUn’unica macchina base gestisce due tipi di materiale. Il linerless è autoadesivo e la sua superficie è siliconata perché il nastro avvolto non si incolli su se stesso; richiede il rullo di stampa DRL con rivestimento antiaderente. L’adesivo InNo-Liner si attiva con l’acqua, non contiene alcun silicone ed è destinato a superfici assorbenti di cartone o carta — tipicamente le etichette di spedizione nella logistica; richiede il rullo DRS siliconato.\n\nLa macchina si basa sulla collaudata HERMES Q: dimensioni, firmware, interfaccia utente, connessioni e attacco per l’applicatore sono identici, e applicatori e supporti di montaggio possono essere spostati tra le due. I tempi ciclo sono quelli della HERMES Q, più circa 50 millisecondi per il taglio.\n\nModelli: HERMES QL4.3 e QL6.3, in versione sinistra e destra, con 200 o 300 dpi. Dati tecnici: stampa termica diretta, 200 o 300 dpi, fino a 250 mm/s, larghezza di stampa fino a 108,4 mm (168 mm su richiesta). Larghezza materiale 50–118 o 50–178 mm, altezza etichetta 30–456 mm, spessore fino a 84 µm. Diametro esterno del rotolo fino a 300 mm su anima da 76 mm — così entrano fino a 800 m di materiale, mentre con materiale su liner nello stesso telaio si hanno un rotolo da 205 mm e 180 m. La taglierina si rimuove per la pulizia senza attrezzi; lama e contro-lama hanno rivestimento antiaderente.', es: 'Una etiqueta linerless no tiene soporte siliconado: el adhesivo está en la propia banda y la máquina corta cada etiqueta a medida. Desaparecen dos cosas a la vez: el residuo del liner y el coste de su eliminación, y la longitud fija de la etiqueta. La HERMES QL corta automáticamente tras la última línea impresa más un desplazamiento definido, de modo que cada etiqueta mide exactamente lo que mide su contenido.\n\nUna sola máquina base admite dos tipos de material. El linerless es autoadhesivo y su superficie está siliconada para que la banda enrollada no se pegue sobre sí misma; requiere el rodillo de impresión DRL con recubrimiento antiadherente. El adhesivo InNo-Liner se activa con agua, no contiene silicona alguna y está pensado para superficies absorbentes de cartón o papel —normalmente etiquetas de envío en logística—; requiere el rodillo DRS siliconado.\n\nLa máquina se basa en la contrastada HERMES Q: dimensiones, firmware, interfaz de usuario, conexiones y acoplamiento del aplicador son idénticos, y los aplicadores y los soportes de montaje pueden trasladarse entre ambas. Los tiempos de ciclo son los de la HERMES Q, más unos 50 milisegundos para el corte.\n\nModelos: HERMES QL4.3 y QL6.3, en versión izquierda y derecha, con 200 o 300 dpi. Datos técnicos: impresión térmica directa, 200 o 300 dpi, hasta 250 mm/s, ancho de impresión hasta 108,4 mm (168 mm bajo pedido). Ancho de material 50–118 o 50–178 mm, altura de etiqueta 30–456 mm, espesor hasta 84 µm. Diámetro exterior del rollo hasta 300 mm sobre núcleo de 76 mm, de modo que caben hasta 800 m de material, mientras que con material sobre liner en el mismo chasis son un rollo de 205 mm y 180 m. La cizalla se extrae para la limpieza sin herramientas; cuchilla y contracuchilla llevan recubrimiento antiadherente.', de: 'Ein linerloses Etikett hat kein silikonisiertes Trägerpapier: Der Klebstoff sitzt auf der Bahn selbst, und die Maschine schneidet jedes Etikett auf Länge. Zwei Dinge entfallen zugleich — der Trägerabfall samt Entsorgungskosten und die feste Etikettenlänge. Der HERMES QL schneidet automatisch nach der letzten Druckzeile plus einem eingestellten Offset, jedes Etikett ist also genau so lang wie sein Inhalt.\n\nEin Grundgerät verarbeitet zwei Materialarten. Linerless ist selbstklebend, die Oberfläche ist silikonisiert, damit die aufgewickelte Bahn nicht an sich selbst haftet; dazu gehört die Druckwalze DRL mit Antihaftbeschichtung. Der InNo-Liner-Klebstoff wird mit Wasser aktiviert, enthält überhaupt kein Silikon und ist für saugfähige Karton- oder Papieroberflächen gedacht — typischerweise Versandetiketten in der Logistik; dazu gehört die silikonbeschichtete Walze DRS.\n\nDie Maschine baut auf dem bewährten HERMES Q auf: Abmessungen, Firmware, Bedienoberfläche, Schnittstellen und Applikatoranbindung sind identisch, Applikatoren und Montagehilfen lassen sich zwischen beiden tauschen. Die Taktraten entsprechen denen des HERMES Q, ergänzt um rund 50 Millisekunden für den Schnitt.\n\nTypen: HERMES QL4.3 und QL6.3, in Links- und Rechtsausführung, mit 200 oder 300 dpi. Technische Daten: Thermodirektdruck, 200 oder 300 dpi, bis 250 mm/s, Druckbreite bis 108,4 mm (168 mm auf Anfrage). Materialbreite 50–118 oder 50–178 mm, Etikettenhöhe 30–456 mm, Dicke bis 84 µm. Rollenaußendurchmesser bis 300 mm auf 76-mm-Kern — so passen bis zu 800 m Material auf die Rolle, während Trägermaterial im selben Chassis eine 205-mm-Rolle und 180 m bedeutet. Das Messer lässt sich zum Reinigen ohne Werkzeug entnehmen; Klinge und Gegenschneide sind antihaftbeschichtet.', ko: '라이너리스 라벨에는 실리콘 처리된 이형지가 없습니다. 접착제가 웹 자체에 있고, 기계가 라벨을 길이에 맞춰 절단합니다. 이로써 두 가지가 동시에 사라집니다. 이형지 폐기물과 그 처리 비용, 그리고 고정된 라벨 길이입니다. HERMES QL은 마지막 인쇄 라인 이후 설정된 오프셋만큼 지나 자동으로 절단하므로, 모든 라벨의 길이가 내용과 정확히 일치합니다.\n\n하나의 기본 장비로 두 가지 소재를 처리합니다. 라이너리스는 점착식이며, 감긴 웹이 서로 달라붙지 않도록 표면이 실리콘 처리되어 있습니다. 여기에는 비점착 코팅의 DRL 인쇄 롤러가 필요합니다. InNo-Liner의 접착제는 물로 활성화되고 실리콘이 전혀 들어 있지 않으며, 흡수성 판지나 종이 표면용으로 주로 물류의 배송 라벨에 쓰입니다. 여기에는 실리콘 코팅 DRS 롤러가 사용됩니다.\n\n이 장비는 검증된 HERMES Q를 기반으로 합니다. 치수, 펌웨어, 사용자 인터페이스, 인터페이스, 어플리케이터 연결부가 동일하며, 어플리케이터와 장착 부속은 두 기종 간에 옮겨 쓸 수 있습니다. 사이클 타임도 HERMES Q와 같으며, 절단을 위해 약 50밀리초가 더해집니다.\n\n기종: HERMES QL4.3과 QL6.3이며, 좌측형과 우측형, 200 또는 300 dpi로 제공됩니다. 기술 사양: 감열 직접 인쇄, 200 또는 300 dpi, 최대 250 mm/s, 인쇄 폭 최대 108.4 mm(168 mm는 별도 요청). 소재 폭 50~118 또는 50~178 mm, 라벨 높이 30~456 mm, 두께 최대 84 µm. 롤 외경 최대 300 mm, 코어 76 mm — 최대 800 m의 소재를 감을 수 있으며, 같은 섀시에 이형지가 있는 소재를 쓰면 205 mm 롤에 180 m입니다. 커터는 공구 없이 분리해 청소할 수 있고, 칼날과 받침날 모두 비점착 코팅입니다.', zh: '无底纸标签没有硅油底纸：胶粘剂直接在料带上，由机器把每张标签切到所需长度。两件事同时消失——底纸废料及其处理成本，以及固定的标签长度。HERMES QL 在最后一行打印内容之后按设定的偏移量自动切断，因此每张标签的长度恰好等于其内容所需。\n\n同一台基本机型可处理两类材料。无底纸材料为自粘型，表面经硅化处理，使卷绕的料带不会自相粘连，需搭配带防粘涂层的 DRL 打印胶辊。InNo-Liner 的胶层由水激活，完全不含硅油，适用于吸水的纸板或纸质表面——通常是物流中的运单标签，需搭配硅油涂层的 DRS 胶辊。\n\n本机以成熟的 HERMES Q 为基础：尺寸、固件、操作界面、接口与贴标模块接口完全相同，贴标模块与安装支架可在两者之间互换。节拍与 HERMES Q 一致，另加约 50 毫秒用于切断。\n\n机型：HERMES QL4.3 与 QL6.3，提供左式与右式，分辨率 200 或 300 dpi。技术参数：热敏直接打印，200 或 300 dpi，最高 250 mm/s，打印宽度最大 108.4 mm（168 mm 需另行询问）。材料宽度 50–118 或 50–178 mm，标签高度 30–456 mm，厚度最大 84 µm。卷料外径最大 300 mm，纸芯 76 mm——最多可容纳 800 m 材料；而在同一机身上使用带底纸的材料则为 205 mm 卷径、180 m。切刀无需工具即可取下清洁，刀片与刀座均为防粘涂层。',
    },
    features: [
      { hu: 'Hordozó nélküli (linerless) és InNo-Liner anyag egy alapgépen', en: 'Linerless and InNo-Liner materials on one base unit', it: 'Materiali linerless e InNo-Liner su un’unica macchina base', es: 'Materiales linerless e InNo-Liner en una sola máquina base', de: 'Linerlose und InNo-Liner-Materialien auf einem Grundgerät', ko: '하나의 기본 장비로 라이너리스와 InNo-Liner 소재 처리', zh: '同一台基本机型处理无底纸与 InNo-Liner 材料' },
      { hu: 'Változó hosszúságú címkék — vágás az utolsó nyomtatott sor után', en: 'Labels of varying length — cut after the final print line', it: 'Etichette di lunghezza variabile — taglio dopo l’ultima riga stampata', es: 'Etiquetas de longitud variable: corte tras la última línea impresa', de: 'Etiketten unterschiedlicher Länge — Schnitt nach der letzten Druckzeile', ko: '가변 길이 라벨 — 마지막 인쇄 라인 이후 절단', zh: '标签长度可变——在最后一行打印内容之后切断' },
      { hu: 'Akár 800 m anyag 300 mm-es tekercsen (hordozóssal 180 m)', en: 'Up to 800 m of material on a 300 mm roll (180 m with liner)', it: 'Fino a 800 m di materiale su un rotolo da 300 mm (180 m con liner)', es: 'Hasta 800 m de material en un rollo de 300 mm (180 m con liner)', de: 'Bis 800 m Material auf einer 300-mm-Rolle (180 m mit Träger)', ko: '300 mm 롤에 최대 800 m 소재(이형지 사용 시 180 m)', zh: '300 mm 卷径最多 800 m 材料（带底纸时为 180 m）' },
      { hu: 'Direkt termikus, 200 / 300 dpi, akár 250 mm/s', en: 'Direct thermal, 200 / 300 dpi, up to 250 mm/s', it: 'Termica diretta, 200 / 300 dpi, fino a 250 mm/s', es: 'Térmica directa, 200 / 300 dpi, hasta 250 mm/s', de: 'Thermodirekt, 200 / 300 dpi, bis 250 mm/s', ko: '감열 직접 인쇄, 200 / 300 dpi, 최대 250 mm/s', zh: '热敏直接打印，200 / 300 dpi，最高 250 mm/s' },
      { hu: 'Nyomtatási szélesség 108,4 mm-ig (168 mm külön kérésre)', en: 'Print width up to 108.4 mm (168 mm on request)', it: 'Larghezza di stampa fino a 108,4 mm (168 mm su richiesta)', es: 'Ancho de impresión hasta 108,4 mm (168 mm bajo pedido)', de: 'Druckbreite bis 108,4 mm (168 mm auf Anfrage)', ko: '인쇄 폭 최대 108.4 mm(168 mm는 별도 요청)', zh: '打印宽度最大 108.4 mm（168 mm 需另行询问）' },
      { hu: 'A HERMES Q applikátorai átvihetők', en: 'HERMES Q applicators can be reused', it: 'Gli applicatori della HERMES Q sono riutilizzabili', es: 'Los aplicadores de la HERMES Q son reutilizables', de: 'Applikatoren des HERMES Q sind weiterverwendbar', ko: 'HERMES Q의 어플리케이터를 그대로 사용 가능', zh: 'HERMES Q 的贴标模块可沿用' },
    ],
    orientation: {
      image: '/images/applicators/hermes-ql-applikator.webp',
      title: {
        hu: 'Ugyanaz a gépváz, ugyanazok az applikátorok',
        en: 'The same chassis, the same applicators', it: 'Lo stesso telaio, gli stessi applicatori', es: 'El mismo chasis, los mismos aplicadores', de: 'Dasselbe Chassis, dieselben Applikatoren', ko: '동일한 섀시, 동일한 어플리케이터', zh: '相同的机身，相同的贴标模块',
      },
      text: {
        hu: 'A méretek, a belső szoftver, a kezelőfelület, a csatlakozók és az applikátor-illesztés megegyeznek a HERMES Q-éval, tehát az applikátorok és a rögzítő elemek a két gép között ide-oda mozgathatók. A ciklusidő is a HERMES Q-é, kiegészítve nagyjából 50 ezredmásodperccel — ennyi a vágás. Aki már HERMES Q-t üzemeltet, nem tanul új gépet, csak új anyagot.',
        en: 'Dimensions, firmware, user interface, interfaces and applicator connectivity match those of the HERMES Q, so applicators and assembly assistances can be moved back and forth between the two machines. Cycle rates are those of the HERMES Q plus roughly 50 milliseconds — that is the cut. Anyone already running a HERMES Q learns no new machine, only a new material.', it: 'Dimensioni, firmware, interfaccia utente, connessioni e attacco per l’applicatore coincidono con quelli della HERMES Q: applicatori e supporti di montaggio si spostano avanti e indietro tra le due macchine. I tempi ciclo sono quelli della HERMES Q più circa 50 millisecondi — è il taglio. Chi già utilizza una HERMES Q non impara una macchina nuova, ma solo un materiale nuovo.', es: 'Las dimensiones, el firmware, la interfaz de usuario, las conexiones y el acoplamiento del aplicador coinciden con los de la HERMES Q, de modo que los aplicadores y los soportes de montaje pueden pasarse de una máquina a otra. Los tiempos de ciclo son los de la HERMES Q más unos 50 milisegundos: eso es el corte. Quien ya opera una HERMES Q no aprende una máquina nueva, solo un material nuevo.', de: 'Abmessungen, Firmware, Bedienoberfläche, Schnittstellen und Applikatoranbindung entsprechen denen des HERMES Q, Applikatoren und Montagehilfen lassen sich also zwischen beiden Maschinen hin- und herbewegen. Die Taktraten sind die des HERMES Q, ergänzt um rund 50 Millisekunden — das ist der Schnitt. Wer bereits einen HERMES Q betreibt, lernt keine neue Maschine, nur ein neues Material.', ko: '치수, 펌웨어, 사용자 인터페이스, 인터페이스, 어플리케이터 연결부가 HERMES Q와 동일하므로 어플리케이터와 장착 부속을 두 기계 사이에서 자유롭게 옮겨 쓸 수 있습니다. 사이클 타임도 HERMES Q와 같고 약 50밀리초가 더해지는데, 그것이 절단 시간입니다. 이미 HERMES Q를 운용하고 있다면 새 기계를 배우는 것이 아니라 새 소재를 배우는 것입니다.', zh: '尺寸、固件、操作界面、接口与贴标模块接口都与 HERMES Q 相同，因此贴标模块与安装支架可在两台机器之间来回调换。节拍同样与 HERMES Q 一致，另加约 50 毫秒——那就是切断所需的时间。已经在用 HERMES Q 的用户不必学一台新机器，只需熟悉一种新材料。',
      },
    },
    variantsLead: {
      hu: 'Az anyag dönt, nem a gép: a két hordozó nélküli anyag ragasztója, nyomóhengere és applikátora is más, és nem cserélhetők egymással.',
      en: 'The material decides, not the machine: the two liner-free materials differ in adhesive, print roller and applicator, and are not interchangeable.', it: 'Decide il materiale, non la macchina: i due materiali senza liner differiscono per adesivo, rullo di stampa e applicatore, e non sono intercambiabili.', es: 'Decide el material, no la máquina: los dos materiales sin liner difieren en adhesivo, rodillo de impresión y aplicador, y no son intercambiables.', de: 'Das Material entscheidet, nicht die Maschine: Die beiden trägerlosen Materialien unterscheiden sich in Klebstoff, Druckwalze und Applikator und sind nicht austauschbar.', ko: '기계가 아니라 소재가 결정합니다. 두 가지 라이너 없는 소재는 접착제, 인쇄 롤러, 어플리케이터가 모두 다르며 서로 호환되지 않습니다.', zh: '决定因素是材料而不是机器：两种无底纸材料在胶粘剂、打印胶辊与贴标模块上均不相同，彼此不可互换。',
    },
    variants: [
      {
        name: 'Linerless',
        purpose: {
          hu: 'Öntapadó, szilikonozott felületű szalag: a ragasztó azonnal fog, a címke bármilyen ipari és logisztikai csomagolásra felkerülhet.',
          en: 'A self-adhesive web with a siliconised surface: the adhesive grips at once, and the label can go onto any industrial or logistics packaging.', us: 'A pressure-sensitive web with a siliconized surface: the adhesive grips at once, and the label can go onto any industrial or logistics packaging.', it: 'Un nastro autoadesivo con superficie siliconata: l’adesivo fa presa subito e l’etichetta può essere applicata su qualsiasi imballaggio industriale e logistico.', es: 'Una banda autoadhesiva de superficie siliconada: el adhesivo agarra de inmediato y la etiqueta puede aplicarse sobre cualquier embalaje industrial y logístico.', de: 'Eine selbstklebende Bahn mit silikonisierter Oberfläche: Der Klebstoff greift sofort, und das Etikett kann auf jede industrielle und logistische Verpackung.', ko: '표면이 실리콘 처리된 점착식 웹입니다. 접착제가 즉시 접착되며, 라벨을 모든 산업 및 물류 포장재에 부착할 수 있습니다.', zh: '表面经硅化处理的自粘料带：胶粘剂即刻附着，标签可贴到任何工业与物流包装上。',
        },
        params: [
          { label: { hu: 'Ragasztó', en: 'Adhesive', it: 'Adesivo', es: 'Adhesivo', de: 'Klebstoff', ko: '접착제', zh: '胶粘剂' }, value: { hu: 'Öntapadó', en: 'Self-adhesive', us: 'Pressure-sensitive', it: 'Autoadesivo', es: 'Autoadhesivo', de: 'Selbstklebend', ko: '점착식', zh: '自粘' } },
          { label: { hu: 'Nyomóhenger', en: 'Print roller', it: 'Rullo di stampa', es: 'Rodillo de impresión', de: 'Druckwalze', ko: '인쇄 롤러', zh: '打印胶辊' }, value: { hu: 'DRL4 / DRL6, tapadásgátló bevonat', en: 'DRL4 / DRL6, anti-stick coating', it: 'DRL4 / DRL6, rivestimento antiaderente', es: 'DRL4 / DRL6, recubrimiento antiadherente', de: 'DRL4 / DRL6, Antihaftbeschichtung', ko: 'DRL4 / DRL6, 비점착 코팅', zh: 'DRL4 / DRL6，防粘涂层' } },
          { label: { hu: 'Applikátor', en: 'Applicator', it: 'Applicatore', es: 'Aplicador', de: 'Applikator', ko: '어플리케이터', zh: '贴标模块' }, value: { hu: 'Mind a HQ applikátor', en: 'All HQ applicators', it: 'Tutti gli applicatori HQ', es: 'Todos los aplicadores HQ', de: 'Alle HQ-Applikatoren', ko: '모든 HQ 어플리케이터', zh: '全部 HQ 贴标模块' } },
          { label: { hu: 'Beépítés', en: 'Mounting', it: 'Montaggio', es: 'Montaje', de: 'Einbau', ko: '설치 방향', zh: '安装方向' }, value: { hu: 'Függőleges vagy vízszintes', en: 'Vertical or horizontal', it: 'Verticale od orizzontale', es: 'Vertical u horizontal', de: 'Vertikal oder horizontal', ko: '수직 또는 수평', zh: '垂直或水平' } },
        ],
      },
      {
        name: 'InNo-Liner',
        purpose: {
          hu: 'Vízzel aktiválódó ragasztó, szilikon egyáltalán nincs benne. Nedvszívó karton- és papírfelületre való — jellemzően logisztikai szállítási címkékhez. Az InNo-Linerhez mikroporlasztó tartozik a vágó mellé: fúvókák nedvesítik meg a ragasztóréteget, desztillált vízzel, egy 6 literes tartályból. A cab adata szerint 100 × 100 mm-es címkénél ez 1000 címkénként 0,2 liter víz.',
          en: 'A water-activated adhesive with no silicone at all. Meant for absorbent cardboard and paper surfaces — typically shipping labels in logistics. InNo-Liner comes with a micro vaporizator next to the cutter: nozzles wet the adhesive layer with distilled water from a 6-litre container. Per cab, for a 100 × 100 mm label that is 0.2 litres of water per 1,000 labels.', us: 'A water-activated adhesive with no silicone at all. Meant for absorbent cardboard and paper surfaces — typically shipping labels in logistics. InNo-Liner comes with a micro vaporizator next to the cutter: nozzles wet the adhesive layer with distilled water from a 6-liter container. Per cab, for a 100 × 100 mm label that is 0.2 liters of water per 1,000 labels.', it: 'Adesivo attivato con acqua, senza alcun silicone. Destinato a superfici assorbenti di cartone e carta — tipicamente etichette di spedizione nella logistica. L’InNo-Liner prevede un micro vaporizzatore accanto alla taglierina: gli ugelli bagnano lo strato adesivo con acqua distillata da un serbatoio da 6 litri. Secondo cab, per un’etichetta da 100 × 100 mm sono 0,2 litri d’acqua ogni 1.000 etichette.', es: 'Adhesivo activado con agua, sin silicona alguna. Pensado para superficies absorbentes de cartón y papel: normalmente etiquetas de envío en logística. El InNo-Liner incorpora un micropulverizador junto a la cizalla: unas boquillas humedecen la capa adhesiva con agua destilada de un depósito de 6 litros. Según cab, para una etiqueta de 100 × 100 mm son 0,2 litros de agua por cada 1.000 etiquetas.', de: 'Ein mit Wasser aktivierter Klebstoff, ganz ohne Silikon. Für saugfähige Karton- und Papieroberflächen — typischerweise Versandetiketten in der Logistik. Zum InNo-Liner gehört ein Mikroverdampfer neben dem Messer: Düsen benetzen die Klebstoffschicht mit destilliertem Wasser aus einem 6-Liter-Behälter. Laut cab sind das bei einem Etikett von 100 × 100 mm 0,2 Liter Wasser je 1.000 Etiketten.', ko: '물로 활성화되는 접착제로, 실리콘이 전혀 들어 있지 않습니다. 흡수성 판지와 종이 표면용이며 주로 물류의 배송 라벨에 사용합니다. InNo-Liner에는 커터 옆에 마이크로 분무기가 함께 제공됩니다. 노즐이 6리터 용기의 증류수로 접착층을 적셔 줍니다. cab의 자료에 따르면 100 × 100 mm 라벨 기준으로 1,000장당 0.2리터의 물이 사용됩니다.', zh: '由水激活的胶粘剂，完全不含硅油。适用于吸水的纸板与纸质表面——通常是物流中的运单标签。 InNo-Liner 在切刀旁配有微雾化器：喷嘴用 6 升容器中的蒸馏水润湿胶层。按 cab 的数据，以 100 × 100 mm 的标签计，每 1000 张耗水 0.2 升。',
        },
        params: [
          { label: { hu: 'Ragasztó', en: 'Adhesive', it: 'Adesivo', es: 'Adhesivo', de: 'Klebstoff', ko: '접착제', zh: '胶粘剂' }, value: { hu: 'Vízzel aktiválódó', en: 'Water-activated', it: 'Attivato con acqua', es: 'Activado con agua', de: 'Mit Wasser aktiviert', ko: '물로 활성화', zh: '水激活' } },
          { label: { hu: 'Nyomóhenger', en: 'Print roller', it: 'Rullo di stampa', es: 'Rodillo de impresión', de: 'Druckwalze', ko: '인쇄 롤러', zh: '打印胶辊' }, value: { hu: 'DRS4 / DRS6, szilikonbevonat', en: 'DRS4 / DRS6, silicone coating', it: 'DRS4 / DRS6, rivestimento siliconico', es: 'DRS4 / DRS6, recubrimiento de silicona', de: 'DRS4 / DRS6, Silikonbeschichtung', ko: 'DRS4 / DRS6, 실리콘 코팅', zh: 'DRS4 / DRS6，硅油涂层' } },
          { label: { hu: 'Applikátor', en: 'Applicator', it: 'Applicatore', es: 'Aplicador', de: 'Applikator', ko: '어플리케이터', zh: '贴标模块' }, value: { hu: 'HQ 4034', en: 'HQ 4034', it: 'HQ 4034', es: 'HQ 4034', de: 'HQ 4034', ko: 'HQ 4034', zh: 'HQ 4034' } },
          { label: { hu: 'Beépítés', en: 'Mounting', it: 'Montaggio', es: 'Montaje', de: 'Einbau', ko: '설치 방향', zh: '安装方向' }, value: { hu: 'Csak függőleges, ütközőléccel', en: 'Vertical only, with a margin stop', it: 'Solo verticale, con arresto laterale', es: 'Solo vertical, con tope lateral', de: 'Nur vertikal, mit Randanschlag', ko: '수직 설치만, 마진 스톱 필요', zh: '仅垂直安装，需配挡边' } },
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    features: [
      ...
      { hu: 'Öntött alumínium alapegység, IP66 / NEMA 12', en: 'Cast-aluminium base unit, IP66 / NEMA 12', us: 'Cast-aluminum base unit, IP66 / NEMA 12', it: 'Unità base in alluminio pressofuso, IP66 / NEMA 12', es: 'Base de aluminio fundido, IP66 / NEMA 12', de: 'Grundeinheit aus Aluminiumguss, IP66 / NEMA 12', ko: '주조 알루미늄 베이스 유닛, IP66 / NEMA 12', zh: '铸铝基座单元，IP66 / NEMA 12' },
  ...
  },
  {
    ...
    brand: 'CAB',
    short: {
      hu: 'Fiber jelölő lézer motoros fókuszállítással — lépcsős, eltérő magasságú felületek jelölésére.',
      en: 'Fibre marking laser with motorised focus shift for stepped surfaces at different heights.', it: 'Laser di marcatura in fibra con spostamento del fuoco motorizzato per superfici a quote diverse.', es: 'Láser de marcaje de fibra con desplazamiento de foco motorizado para superficies a distintas alturas.', de: 'Faser-Markierlaser mit motorischer Fokusverschiebung für Flächen auf unterschiedlichen Höhen.', ko: '높이가 다른 단차 면을 마킹하는 전동 초점 이동식 파이버 마킹 레이저.', zh: '带电动焦点位移的光纤打标激光器，适用于不同高度的阶梯面。',
    },
    description: {
      hu: 'A XENO 4S a XENO 4 azon változata, amelyben a fókusztávolság motoros állítású. Ez egyetlen dolgot old meg, de azt alapjában: ha a munkadarabon eltérő magasságú felületek vannak — lépcsős öntvény, süllyesztett fészek, bordázott ház —, a hagyományos lézernek minden szinthez újra kellene fókuszálnia, vagy a darabot kellene mozgatni. A 4S a fókuszt a másodperc törtrésze alatt tolja el, mechanikus egységállítással, így a több szintet érintő jelölés CIKLUSIDŐ-VESZTESÉG NÉLKÜL készül el. A vezérlő a mindenkori síkhoz automatikusan újraszámolja a rajzolat méretarányát, tehát a jelölés minden szinten azonos méretű marad. Az eltolás a 160.2-es objektívvel ±35 mm, a 254.2-essel ±70 mm — utóbbi 140 mm-es magasságkülönbséget hidal át. A jelölőmező az eltolással együtt változik: 254.2-essel 160×160 mm-től (+70) 250×250 mm-ig (−70). Minden másban a XENO 4-gyel azonos: 20, 30 vagy 50 W-os, dióda-pumpált, léghűtéses ytterbium fiber lézer, cabLase szoftver, Ipar 4.0, TCP/IP. A XENO 4S+ IPG lézerforrással és 2+2 év garanciával érkezik.',
      en: 'The XENO 4S is the XENO 4 variant with a motorised focal distance. It solves one thing, but solves it fundamentally: where a workpiece has surfaces at different heights — a stepped casting, a recessed seat, a ribbed housing — a conventional laser would have to refocus for each level, or the part would have to be moved. The 4S shifts the focus within a fraction of a second by mechanical unit adjustment, so markings spanning several levels are produced WITHOUT LOSS OF CYCLE TIME. The control unit automatically recalculates the layout scaling for the respective plane, so the marking stays the same size on every level. The shift is ±35 mm with the 160.2 lens and ±70 mm with the 254.2 — the latter bridging a 140 mm height difference. The marking field changes with the shift: with the 254.2 from 160×160 mm (+70) to 250×250 mm (−70). In every other respect it matches the XENO 4: a 20, 30 or 50 W diode-pumped, air-cooled ytterbium fibre laser, cabLase software, Industry 4.0, TCP/IP. The XENO 4S+ comes with an IPG beam source and a 2+2-year warranty.', it: 'Lo XENO 4S è la variante dello XENO 4 con distanza focale motorizzata. Risolve una cosa sola, ma in modo sostanziale: quando un pezzo presenta superfici a quote diverse — una fusione a gradini, una sede ribassata, una carcassa nervata — un laser tradizionale dovrebbe rifocalizzare per ogni livello, oppure occorrerebbe spostare il pezzo. Il 4S sposta il fuoco in una frazione di secondo mediante regolazione meccanica dell’unità, così le marcature che interessano più livelli vengono eseguite SENZA PERDITA DI TEMPO CICLO. L’unità di controllo ricalcola automaticamente la scala del layout per il piano interessato, quindi la marcatura mantiene la stessa dimensione su ogni livello. Lo spostamento è di ±35 mm con la lente 160.2 e di ±70 mm con la 254.2, che copre una differenza di altezza di 140 mm. Il campo di marcatura varia con lo spostamento: con la 254.2 da 160×160 mm (+70) a 250×250 mm (−70). Per tutto il resto è identico allo XENO 4: laser in fibra allo itterbio da 20, 30 o 50 W, pompato a diodi e raffreddato ad aria, software cabLase, Industry 4.0, TCP/IP. Lo XENO 4S+ è dotato di sorgente IPG e garanzia 2+2 anni.', es: 'El XENO 4S es la variante del XENO 4 con distancia focal motorizada. Resuelve una sola cosa, pero de raíz: cuando una pieza tiene superficies a distintas alturas —una fundición escalonada, un alojamiento rebajado, una carcasa nervada—, un láser convencional tendría que reenfocar en cada nivel, o habría que mover la pieza. El 4S desplaza el foco en una fracción de segundo mediante ajuste mecánico de la unidad, de modo que los marcajes que abarcan varios niveles se realizan SIN PÉRDIDA DE TIEMPO DE CICLO. La unidad de control recalcula automáticamente la escala del diseño para el plano correspondiente, por lo que el marcaje conserva el mismo tamaño en todos los niveles. El desplazamiento es de ±35 mm con la lente 160.2 y de ±70 mm con la 254.2, que salva una diferencia de altura de 140 mm. El campo de marcaje varía con el desplazamiento: con la 254.2, de 160×160 mm (+70) a 250×250 mm (−70). En todo lo demás coincide con el XENO 4: láser de fibra de iterbio de 20, 30 o 50 W, bombeado por diodo y refrigerado por aire, software cabLase, Industria 4.0, TCP/IP. El XENO 4S+ incorpora fuente IPG y garantía de 2+2 años.', de: 'Der XENO 4S ist die XENO-4-Variante mit motorischer Fokusverstellung. Er löst eine einzige Sache, diese aber grundlegend: Hat ein Werkstück Flächen auf unterschiedlichen Höhen — ein gestufter Guss, eine abgesetzte Aufnahme, ein verripptes Gehäuse —, müsste ein herkömmlicher Laser für jede Ebene neu fokussieren, oder das Teil müsste bewegt werden. Der 4S verschiebt den Fokus im Bruchteil einer Sekunde durch mechanische Einheitenverstellung, sodass Markierungen über mehrere Ebenen OHNE VERLUST AN TAKTZEIT entstehen. Die Steuereinheit berechnet die Layout-Skalierung für die jeweilige Ebene automatisch neu, die Markierung bleibt also auf jeder Ebene gleich groß. Die Verschiebung beträgt mit der Linse 160.2 ±35 mm und mit der 254.2 ±70 mm — Letztere überbrückt 140 mm Höhenunterschied. Das Markierfeld ändert sich mit der Verschiebung: mit der 254.2 von 160×160 mm (+70) bis 250×250 mm (−70). In allem Übrigen entspricht er dem XENO 4: 20, 30 oder 50 W diodengepumpter, luftgekühlter Ytterbium-Faserlaser, cabLase Software, Industrie 4.0, TCP/IP. Der XENO 4S+ kommt mit IPG-Strahlquelle und 2+2 Jahren Garantie.', ko: 'XENO 4S는 초점 거리를 전동으로 조절하는 XENO 4 변형 모델입니다. 한 가지 문제를 해결하지만, 근본적으로 해결합니다. 공작물에 높이가 다른 면이 있는 경우 — 단차가 있는 주물, 함몰된 시트, 리브가 있는 하우징 — 일반 레이저는 각 단마다 다시 초점을 맞추거나 부품을 이동해야 합니다. 4S는 기계적 유닛 조절로 초점을 순식간에 이동시키므로, 여러 단에 걸친 마킹도 사이클 타임 손실 없이 완료됩니다. 컨트롤 유닛이 해당 평면에 맞춰 레이아웃 배율을 자동으로 재계산하므로 마킹 크기는 모든 단에서 동일하게 유지됩니다. 이동량은 160.2 렌즈에서 ±35 mm, 254.2 렌즈에서 ±70 mm이며, 후자는 140 mm의 높이 차를 커버합니다. 마킹 필드는 이동량에 따라 달라집니다: 254.2 렌즈에서 160×160 mm(+70)부터 250×250 mm(−70)까지. 그 밖의 사양은 XENO 4와 동일합니다: 20, 30 또는 50 W 다이오드 펌핑 공랭식 이터븀 파이버 레이저, cabLase 소프트웨어, Industry 4.0, TCP/IP. XENO 4S+는 IPG 빔 소스와 2+2년 보증을 제공합니다.', zh: 'XENO 4S 是配备电动焦距调节的 XENO 4 型号。它只解决一个问题，但解决得很彻底：当工件存在不同高度的表面时——阶梯铸件、下沉式安装座、带筋外壳——传统激光器必须为每个层面重新对焦，或者必须移动工件。4S 通过机械单元调节，在几分之一秒内完成焦点位移，因此跨越多个层面的打标可以在不损失节拍时间的情况下完成。控制单元会自动为相应平面重新计算图案缩放比例，因此打标在每个层面上尺寸保持一致。使用 160.2 透镜时位移为 ±35 mm，使用 254.2 透镜时为 ±70 mm，后者可跨越 140 mm 的高度差。打标范围随位移变化：使用 254.2 透镜时从 160×160 mm（+70）到 250×250 mm（−70）。其余方面与 XENO 4 相同：20、30 或 50 W 二极管泵浦、风冷镱光纤激光器，cabLase 软件，工业 4.0，TCP/IP。XENO 4S+ 版本配备 IPG 光束源并提供 2+2 年保修。',
    },
    features: [
      { hu: 'Motoros fókuszállítás: ±35 mm (160.2) vagy ±70 mm (254.2)', en: 'Motorised focus shift: ±35 mm (160.2) or ±70 mm (254.2)', it: 'Spostamento del fuoco motorizzato: ±35 mm (160.2) o ±70 mm (254.2)', es: 'Desplazamiento de foco motorizado: ±35 mm (160.2) o ±70 mm (254.2)', de: 'Motorische Fokusverschiebung: ±35 mm (160.2) oder ±70 mm (254.2)', ko: '전동 초점 이동: ±35 mm(160.2) 또는 ±70 mm(254.2)', zh: '电动焦点位移：±35 mm（160.2）或 ±70 mm（254.2）' },
      { hu: '140 mm magasságkülönbség áthidalása újrafókuszálás nélkül', en: 'Bridges a 140 mm height difference without refocusing', it: 'Supera 140 mm di differenza di altezza senza rifocalizzare', es: 'Salva 140 mm de diferencia de altura sin reenfocar', de: 'Überbrückt 140 mm Höhenunterschied ohne Nachfokussieren', ko: '재초점 없이 140 mm 높이 차 대응', zh: '无需重新对焦即可跨越 140 mm 高度差' },
      { hu: 'Több szint jelölése ciklusidő-veszteség nélkül', en: 'Marks several levels without loss of cycle time', it: 'Marca più livelli senza perdita di tempo ciclo', es: 'Marca varios niveles sin pérdida de tiempo de ciclo', de: 'Markiert mehrere Ebenen ohne Verlust an Taktzeit', ko: '사이클 타임 손실 없이 여러 단 마킹', zh: '跨多个层面打标而不损失节拍时间' },
      { hu: 'A vezérlő síkonként újraszámolja a méretarányt', en: 'The control unit rescales the layout for each plane', it: 'L’unità di controllo riscala il layout per ogni piano', es: 'La unidad de control reescala el diseño para cada plano', de: 'Die Steuereinheit skaliert das Layout je Ebene neu', ko: '컨트롤 유닛이 평면별로 배율 재계산', zh: '控制单元按平面重新计算缩放比例' },
      { hu: 'Eltolási sebesség 0,5 mm/ms (160.2), 0,3 mm/ms (254.2)', en: 'Shift speed 0.5 mm/ms (160.2), 0.3 mm/ms (254.2)', it: 'Velocità di spostamento 0,5 mm/ms (160.2), 0,3 mm/ms (254.2)', es: 'Velocidad de desplazamiento 0,5 mm/ms (160.2), 0,3 mm/ms (254.2)', de: 'Verschiebegeschwindigkeit 0,5 mm/ms (160.2), 0,3 mm/ms (254.2)', ko: '이동 속도 0.5 mm/ms(160.2), 0.3 mm/ms(254.2)', zh: '位移速度 0.5 mm/ms（160.2）、0.3 mm/ms（254.2）' },
      { hu: '20 / 30 / 50 W ytterbium fiber lézer, léghűtéses', en: '20 / 30 / 50 W ytterbium fibre laser, air-cooled', it: 'Laser in fibra allo itterbio da 20 / 30 / 50 W, raffreddato ad aria', es: 'Láser de fibra de iterbio de 20 / 30 / 50 W, refrigerado por aire', de: '20 / 30 / 50 W Ytterbium-Faserlaser, luftgekühlt', ko: '20 / 30 / 50 W 이터븀 파이버 레이저, 공랭식', zh: '20 / 30 / 50 W 镱光纤激光器，风冷' },
      { hu: 'Jelölőmező 100×100 – 250×250 mm (objektív és eltolás szerint)', en: 'Marking field 100×100 – 250×250 mm (lens and shift dependent)', it: 'Campo di marcatura 100×100 – 250×250 mm (secondo lente e spostamento)', es: 'Campo de marcaje 100×100 – 250×250 mm (según lente y desplazamiento)', de: 'Markierfeld 100×100 – 250×250 mm (linsen- und verschiebungsabhängig)', ko: '마킹 필드 100×100 – 250×250 mm(렌즈 및 이동량에 따라)', zh: '打标范围 100×100 – 250×250 mm（视透镜与位移而定）' },
      { hu: 'cabLase szoftver, Ipar 4.0, TCP/IP vezérlés', en: 'cabLase software, Industry 4.0, TCP/IP control', it: 'Software cabLase, Industry 4.0, controllo TCP/IP', es: 'Software cabLase, Industria 4.0, control por TCP/IP', de: 'cabLase Software, Industrie 4.0, TCP/IP-Steuerung', ko: 'cabLase 소프트웨어, Industry 4.0, TCP/IP 제어', zh: 'cabLase 软件，工业 4.0，TCP/IP 控制' },
    ],
    image: '/images/products/cab-xeno-4.jpg',
    datasheet: '/datasheets/cab-laser-xeno.pdf',
    figure: {
  ...
  },
  {
    ...
    features: [
      ...
      { hu: '20 / 30 W fiber lézer', en: '20 / 30 W fibre laser', it: 'Laser in fibra da 20 / 30 W', es: 'Láser de fibra de 20 / 30 W', de: '20 / 30 W Faserlaser', ko: '20 / 30 W 파이버 레이저', zh: '20 / 30 W 光纤激光器' },
  ...
  },
  {
    ...
    features: [
      ...
      { hu: '4,5" színes érintőkijelző', en: '4.5" colour touchscreen', it: 'Touchscreen a colori da 4,5"', es: 'Pantalla táctil en color de 4,5"', de: '4,5"-Farb-Touchscreen', ko: '4.5" 컬러 터치스크린', zh: '4.5" 彩色触摸屏' },
  ...
  // ——— TYKMA Electrox — lézeres jelölő- és gravírozórendszerek ———
  {
    ...
    features: [
      ...
      { hu: 'MOPA fiber lézer, 20 / 50 W', en: 'MOPA fibre laser, 20 / 50 W', it: 'Laser in fibra MOPA, 20 / 50 W', es: 'Láser de fibra MOPA, 20 / 50 W', de: 'MOPA-Faserlaser, 20 / 50 W', ko: 'MOPA 파이버 레이저, 20 / 50 W', zh: 'MOPA 光纤激光器，20 / 50 W' },
  ...
  },
  {
    ...
    features: [
      ...
      { hu: 'MOPA fiber lézer, 20–100 W', en: 'MOPA fibre laser, 20–100 W', it: 'Laser in fibra MOPA, 20–100 W', es: 'Láser de fibra MOPA, 20–100 W', de: 'MOPA-Faserlaser, 20–100 W', ko: 'MOPA 파이버 레이저, 20–100 W', zh: 'MOPA 光纤激光器，20–100 W' },
  ...
  },
  {
    ...
    features: [
      ...
      { hu: 'MOPA fiber lézerforrás', en: 'MOPA fibre laser source', it: 'Sorgente laser in fibra MOPA', es: 'Fuente láser de fibra MOPA', de: 'MOPA-Faserlaserquelle', ko: 'MOPA 파이버 레이저 소스', zh: 'MOPA 光纤激光源' },
  ...
  },
  {
    ...
    features: [
      ...
      { hu: 'Anodizált alumínium ház', en: 'Anodised aluminium housing', it: 'Alloggiamento in alluminio anodizzato', es: 'Carcasa de aluminio anodizado', de: 'Gehäuse aus eloxiertem Aluminium', ko: '아노다이징 알루미늄 하우징', zh: '阳极氧化铝外壳' },
    ],
  },
  {
    slug: 'start-ld3500',
  ...
  },
  {
    ...
    short: {
      ...
      en: 'For laser coding and marking: 400 m³/h, colour touch screen, 100–230 V anywhere in the world.', us: 'For laser coding and marking: 400 m³/h, color touch screen, 100–230 V anywhere in the world.', it: 'Per codifica e marcatura laser: 400 m³/h, touch screen a colori, 100–230 V in tutto il mondo.', es: 'Para codificación y marcaje láser: 400 m³/h, pantalla táctil en color, 100–230 V en cualquier parte del mundo.', de: 'Für Lasercodierung und -beschriftung: 400 m³/h, Farb-Touchscreen, 100–230 V weltweit.', ko: '레이저 코딩 및 마킹용: 400 m³/h, 컬러 터치스크린, 전 세계 100–230 V 대응.', zh: '用于激光喷码与打标：400 m³/h，彩色触摸屏，全球 100–230 V 通用。',
    },
    description: {
